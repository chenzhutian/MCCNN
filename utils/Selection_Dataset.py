...
class SelectionDataSet(with_metaclass(abc.ABCMeta)):
    ...
    Attributes:
        ...
        recordList_ (array of strings): List of paths of the models of the dataset.
    ...
    def _load_model_(self, record_idx):
        """Method to load a model from cache.

        Args:
            record_idx (string): Path to the model that needs to be loaded.
    ...
    def start_iteration(self):
        ...
        """
        self.recordList_ = self.randomState_.permutation(self.recordList_)
        self.iterator_ = 0
    ...
    def has_more_batches(self):
        ...
        Returns:
            more (bool): Boolean that indicates that there are more models
                to process.
        """
        return self.iterator_ < len(self.recordList_)
    ...
    def get_next_batch(self, repeatModelInBatch = False):
        ...
        accumLabels = np.array([]) if self.pointLabels_ else None
        accumCat = np.array([]) if self.useCategories_ else None
        accumPaths = []
        ...
        for i in range(self.batchSize_):
            ...
            if self.iterator_ < len(self.recordList_):
            
                # Check if the model fit in the batch.
                currRecordIdx = self.recordList_[self.iterator_]
                currModelNumPts = self.numPts_[currRecordIdx]
                ...
                if (self.maxPtsxBatch_ == 0) or ((numPtsInBatch+currModelNumPts) <= self.maxPtsxBatch_):
                    ...
                    currModelCat = self.categories_[currRecordIdx] if self.useCategories_ else None
                    ...
                    accumPaths.append(currRecordIdx)
